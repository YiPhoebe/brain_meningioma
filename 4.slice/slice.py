...
def save_filtered_slices(
    ...
    def padding(arr, target_shape):
        h, w = arr.shape
        th, tw = target_shape
        pad_h = max((th - h) // 2, 0)
        pad_w = max((tw - w) // 2, 0)
        min_val = arr.min()
        padded = np.pad(arr, ((pad_h, th - h - pad_h), (pad_w, tw - w - pad_w)), mode='constant', constant_values=min_val)
        return padded
    ...
    for i in keep_idx:
        ...
        vol_slice = padding(vol_slice, target_shape)
        mask_slice = padding(mask_slice, target_shape)
